...
```{r plot_data_1}
Criteria <- c("fdp","dp")
plot_data <- function(data, criteria, title.main){
  data <- 
    data %>% 
    filter( Criteria %in% criteria) 
  data <- melt(data,  id=c("Criteria","drug"))
  data <- reshape2::dcast(data, drug+variable~Criteria,
                          value.var = "value")
  p <- 
...
p.pos <- plot_data(plot_data_pos, Criteria, "Position")
p.posmut <- plot_data(plot_data_posmut, Criteria,"Position+Direction")
...
knockoff_and_bhq.combine <- function (X, Y, q) {
  ...
  BH.selected <- colnames(X)[which(BH.group==1)]
